import { projectLinks } from "@/components/common/helper";
import { BackarrowIcon } from "@/components/common/icons";
import Link from "next/link";
import React from "react";

const Projectsidebar = ({ toggleProject }) => {
  return (
    <div className="bg-fllopydisk flex flex-col justify-between pb-10 px-6">
      <div className="flex flex-col gap-[48px]">
        <div className="py-4 flex gap-6 items-center">
          ...
          <div>
            <button type="submit" className="order-1" onClick={toggleProject}>
              <BackarrowIcon />
            </button>
          </div>
        </div>
        <div className="flex flex-col gap-6">
          {projectLinks.map((items, index) => (
